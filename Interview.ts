function getDataWawancara() {
  srcSheetName = "CompileWawancara";
  ...
  destSheet = destinationSpreadsheet.getSheetByName('RekapWWCR');
  ...
  for (i = 0; i < listFile.length; i++) {
    ...
    sourceAspectFromAssesor = sourceSheet.getRange('D12:W12').getValues();
    ...
    if (sourceName[0][0] !== nameBefore) {
      ...
      aspectFromAssesorCol = destSheet.getRange('E' + index + ':X' + index);
    } else {
      noCol = destSheet.getRange('A' + index + ':A' + index);
      nameCol = destSheet.getRange('B' + index + ':B' + index);
      assesorCol = destSheet.getRange('D' + index + ':D' + index);
      aspectFromAssesorCol = destSheet.getRange('Y' + index + ':AR' + index);
    }
